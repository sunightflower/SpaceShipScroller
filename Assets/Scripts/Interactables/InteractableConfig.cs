...
namespace Interactables
{
    ...
    {
        [SerializeField] private List<InteractablePrefabData> interactablePrefabsData;

        public Interactable Get(InteractableType type, int grade)
        {
            var interactable = interactablePrefabsData.FirstOrDefault(
                x => x.InteractableType == type && x.InteractableGrade == grade);
